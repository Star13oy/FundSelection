...
- 修复详情页点击后后端 `500` 的问题，详情接口已恢复正常返回。
- 选基页接入真实分页，并补充基金规模显示。
- 首页与选基页接入真实 `近1年收益`、`最大回撤` 字段。
- 板块热度分布改为实时数据接口，不再使用静态假数据。
- 风险偏好切换和行情刷新接口做了缓存与异步优化，降低等待时间。
- 前后端开发端口固定为 `5173/8000`，避免调试时自动漂移。

### 当前验收重点

- 首页推荐列表、选基列表、详情页已统一使用同一套基金评分字段。
- 颜色规则按 A 股展示逻辑执行：
  - 正收益红色
  - 负收益绿色
  - 回撤按同一规则呈现
- 选基页支持分页浏览大盘基金池，而非一次性加载全部结果。

### 本轮新增/调整接口

- `GET /api/v1/funds`
  - 新增字段：`fund_type`、`scale_billion`、`one_year_return`、`max_drawdown`
- `GET /api/v1/funds/{code}`
  - 修复详情页序列化逻辑
- `GET /api/v1/market/sector-heat`
  - 返回板块热度实时数据
